...
class Classifier(object):
    ...
    def train(self, epoch):
        ...
        [{remaining} {rate_fmt} {postfix}%]"
        pbar = tqdm(total=len(self.trainloader), bar_format=barformat, postfix={"accuracy":acc})
        for batch_idx, (data, target) in enumerate(self.trainloader):
            pbar.set_description("Epoch {0} training".format(epoch+1))
